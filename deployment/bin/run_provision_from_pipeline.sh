...
DROPLET_ID=$(doctl compute droplet create --size $SIZE --image ubuntu-24-04-x64 --region fra1 --ssh-keys $SSH_FINGERPRINT --wait --no-header --format=ID --access-token $DOCTL_TOKEN $NAME)
